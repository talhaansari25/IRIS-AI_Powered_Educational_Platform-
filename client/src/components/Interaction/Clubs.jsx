...
function InMsg() {
    const fetch2 = (query) => {
        ...
        fetch('http://127.0.0.1:8080/askcounsellor', { // Update this with your backend URL if needed
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.response) {
                setmymsg((prevMessages) => [
                    ...prevMessages,
                    { pos: 'left', message: data.response }
                ]);
            } else {
                setmymsg((prevMessages) => [
                    ...prevMessages,
                    { pos: 'left', message: "Sorry, I couldn't understand that." }
                ]);
            }
            setMsgcache('');
            setTyping(false);
        ...
        .catch(error => {
            ...
            setmymsg((prevMessages) => [
                ...prevMessages,
                { pos: 'left', message: "Sorry, there was an error. Please try again later." }
            ]);
        });
    };
